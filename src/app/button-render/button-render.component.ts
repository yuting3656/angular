...
export class ButtonRenderComponent implements OnInit {
  ...
  delete(){
    let deleteUrl = this.url + this.renderValue
    console.log(deleteUrl)
    if(confirm("你確定要刪除?")){
      this.service.httpDelete(deleteUrl).subscribe((data)=> console.log("Deleted!!"),(error)=> console.log(error), ()=>this.router.navigate(['/smartTable']) )
